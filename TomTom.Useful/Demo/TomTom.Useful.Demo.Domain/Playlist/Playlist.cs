...
namespace TomTom.Useful.Demo.Domain.Playlist
{
        ...
        , IEmitsEvent<PlaylistPublished>
    {
        ...
        {
            yield return this.EmitEvent(PlaylistEventFactory.PlaylistCreated(command.MessageId, command.CorrelationId, command.Title, command.ExplorerId, command.CreationTimestamp));
        }

        public IEnumerable<Event<PlaylistIdentity>> Publish(PublishPlaylistCommand command)
        {
            yield return this.EmitEvent(PlaylistEventFactory.PlaylistPublished(this.Id, this.Version, command.MessageId, command.CorrelationId));
        }
